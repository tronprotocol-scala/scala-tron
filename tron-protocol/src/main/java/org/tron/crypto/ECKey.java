...
public class ECKey implements Serializable {
    ...
    /**
     * Equal to CURVE.getN().shiftRight(1), used for canonicalising the S
    ...
    /**
     * Utility for compressing an elliptic curve point. Returns the same
     * point if it's already compressed.
    ...
    /**
     * Utility for decompressing an elliptic curve point. Returns the same
     * point if it's already compressed.
     * See the ECKey class docs for a discussion of point compression.
     *
     * @param compressed -
     ...
     * really the result of multiplying the
     * generator point by the private key. This is used to speed things up
     * when you know you have the right values
     ...
     * signatures, from the given point. The
    ...
    /**
     * Creates an ECKey that cannot be used for signing, only verifying
     * signatures, from the given encoded point.
     ...
     * uncompressed form. Normally you would
     * never need this: it's for specialised scenarios or when backwards
     ...
     * This may be because it was never
    ...
    /**
     * Gets the private key in the form of an integer field element. The
     * public key is derived by performing EC
     ...
     * Unless you absolutely need the private key it is better for security
     * reasons to just use toString().
     *
     * @return -
     */
    ...
    /**
     * <p>Verifies the given ECDSA signature against the message bytes using
     * the public key bytes.</p>
     * <p>
     * <p>When using native ECDSA verification, data must be 32 bytes, and no
     * element may be
    ...
    /**
     * Verifies the given ASN.1 encoded ECDSA signature against a hash using
     * the public key.
     *
     * @param data      Hash of the data to verify.
     * @param signature signature.
    ...
    /**
     * Verifies the given R/S pair (signature) against a hash using the
     * public key.
     *
     * @param sigHash   -
     * @param signature -
    ...
    /**
     * Returns true if this pubkey is canonical, i.e. the correct length
    ...
    /**
     * <p>Given the components of a signature and a selector value, recover
     * and return the public key
     * that generated the signature according to the algorithm in SEC1v2
     * section 4.1.6.</p>
     * <p>
     ...
     * <p>
     * <p>Given the above two points, a correct usage of this method is
     * inside a for loop from 0 to 3, and if the
     * output is null OR a key that is not the one you expect, you try again
    ...
    /**
     * Returns a 32 byte array containing the private key, or null if the key
     * is encrypted or public only
     *
     * @return -
     */
    @Nullable
